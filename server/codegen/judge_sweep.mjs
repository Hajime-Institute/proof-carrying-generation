...
const JUDGES = [
  { name: "gemini-2.5-flash", make: () => google("gemini-2.5-flash"), temp: 1 },
  { name: "gpt-4o",           make: () => openai("gpt-4o"),           temp: 1 },
  { name: "gemini-2.5-pro",   make: () => google("gemini-2.5-pro"),   temp: 1 },
...
async function ask(judge, question) {
  try {
    const opts = { model: judge.make(), maxOutputTokens: 16000,
      prompt: `You are an expert code reviewer. Answer with YES or NO as the very first word.\n${question}` };
    if (judge.temp != null) opts.temperature = judge.temp;
    const r = await generateText(opts);
    const t = (r.text || "").trim();
    const m = t.match(/\b(yes|no)\b/i);
...
for (const judge of JUDGES) {
  ...
  const rows = await pool(obls, 6, async (o) => {
    const q = `Specification: ${o.desc}\n\nCandidate implementation:\n\`\`\`python\n${o.code}\n\`\`\`\nClaim: this implementation ${o.universal}. Does the claim hold? Remember: answer YES or NO.`;
...
const perJudge = records.map(r => {
  ...
  for (const row of r.rows) {
    if (row.majority === null) abstain++; else confident++;
...
writeFileSync(HERE + "judge_sweep_results.json", JSON.stringify({ summary, byObl, records }, null, 2));
