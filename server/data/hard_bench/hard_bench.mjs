...
mkdirSync("/tmp/hard", { recursive: true });
...
const MODELS = [
  { id: "gpt-4o-mini",      tier: "weak",   m: openai("gpt-4o-mini") },
  { id: "gpt-4o",          tier: "strong-oai", m: openai("gpt-4o") },
  { id: "gemini-3.5-flash", tier: "current", m: google("gemini-3.5-flash"), opts: { providerOptions: { google: { thinkingConfig: { thinkingBudget: 0 } } } } },
];
...
async function genHTML(model, prompt, temp = 0.4) {
  for (let a = 1; a <= 3; a++) {
    try {
      const params = { model: model.m, messages: [{ role: "user", content: prompt }], maxOutputTokens: 20000, maxRetries: 2, ...(model.opts || {}) };
...
async function certify(app, html) {
  const path = `/tmp/hard/_${Math.random().toString(36).slice(2)}.html`; writeFileSync(path, html);
...
const REPAIR = process.env.NO_REPAIR ? 0 : 1; // one certify-or-repair step by default
let results = [];
try { results = JSON.parse(readFileSync("/tmp/hard/results.json", "utf8")).filter((r) => !r.error); } catch {}
const doneKeys = new Set(results.map((r) => r.app + "/" + r.model));
if (doneKeys.size) console.log(`[resume] ${doneKeys.size} cells already done`);
console.log(`[hard-bench] ${APPS.length} apps x ${MODELS.length} models; repair steps=${REPAIR}\n`);
for (const app of APPS) {
  for (const model of MODELS) {
    if (doneKeys.has(app.key + "/" + model.id)) continue;
    let html = await genHTML(model, genPrompt(app));
    if (!html) { console.log(`  ${app.key}/${model.id}: GEN FAIL`); results.push({ app: app.key, model: model.id, tier: model.tier, error: true }); writeFileSync("/tmp/hard/results.json", JSON.stringify(results, null, 2)); continue; }
    let V = await certify(app, html); const s0 = summarize(V);
    // one certify-or-repair step on the hard obligations
    let sR = s0, repaired = false;
    if (REPAIR && s0.ref > 0) {
      const fixed = await genHTML(model, repairPrompt(app, html, V), 0.3);
      if (fixed) { const V2 = await certify(app, fixed); sR = summarize(V2); if (sR.cert >= s0.cert) { html = fixed; V = V2; repaired = true; } else sR = s0; }
    }
    writeFileSync(`/tmp/hard/${app.key}_${model.id}.html`, html);
    const refs = Object.entries(V).filter(([, o]) => o.verdict === "refuted").map(([k]) => k);
    results.push({ app: app.key, model: model.id, tier: model.tier, certPre: s0.cert, refPre: s0.ref, ...sR, repaired, refuted: refs, V });
    const byStr = Object.entries(sR.by).map(([k, [c, r]]) => `${k} ${c}/${c + r}`).join("  ");
    console.log(`  ${app.key}/${model.id.padEnd(16)} pre ${s0.cert}/${s0.total} -> post ${sR.cert}/${sR.total} ${repaired ? "(repaired)" : ""}  [${byStr}]  refuted: ${refs.join(", ") || "none"}`);
    writeFileSync("/tmp/hard/results.json", JSON.stringify(results, null, 2));
  }
  console.log("");
}
await browser.close();
...
console.log("\nresults -> /tmp/hard/results.json ; generated apps -> /tmp/hard/*.html");
